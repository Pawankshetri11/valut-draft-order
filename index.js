...
app.get('/create-draft-order', async (req, res) => {
  try {
    ...
    if (!invoice_url) {
      return res.status(500).send('❌ No invoice URL received');
    }
...
const PORT = 3000;
app.listen(PORT, () => {
  console.log(`✅ Vault Checkout API is running on port ${PORT}`);
});
